...
public class percolationboard {
	...
	private QuickUnionPathCompression nodes;
	private double totalspots, filledspots;
	private boolean printboard;
	public percolationboard(int n, boolean printboard)
	{
	...
	public void fill()
	{
		...
		while (nodes.connected(0, n*n+1) == false && ijlist.size()!=0)
		{
			...
			if (rowfill != 0 )
			{
				if ( board[rowfill-1][colfill] == true)nodes.merge(nodeIndex,nodeIndexUp);
			}
			if (rowfill != n-1 )
			{
				if (board[rowfill+1][colfill] == true)nodes.merge(nodeIndex, nodeIndexDown);
			}
			if (colfill != n-1)
			{
				if (board[rowfill][colfill+1] == true)nodes.merge(nodeIndex, nodeIndexRight);
			}
			if (colfill != 0 )
			{
				if (board[rowfill][colfill-1] == true)nodes.merge(nodeIndex, nodeIndexLeft);
			}
